...
const Shipping = () => {
    ...
    
    {
                ...
                <h2 className='text-slate-600 font-semibold pb-2'>Deliver To {state.name}</h2>
                    ...
                    <p className='text-slate-600 text-sm' >Email To PesceIsland@gmail.com</p>   
                </div>
    }
    </div>
...
<div className='bg-white p-3 text-slate-600 flex flex-col gap-3'>
  ...
  <div className='flex justify-between items-center'>
      <span>Total</span>
      <span className='text-lg text-[#059473]'>Rp.{price + shipping_fee} </span>
  </div>
  <button onClick={placeOrder} disabled={res ? false : true} className={`px-5 py-[6px] rounded-sm hover:shadow-red-500/50 hover:shadow-lg ${res ? 'bg-red-600' : 'bg-slate-500'}  text-sm text-white uppercase cursor-pointer`}>
     Place Order 
  </button>
